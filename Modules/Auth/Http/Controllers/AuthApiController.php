...
class AuthApiController extends Controller
{
    ...
    public function __construct(User $user,UserRepository $userRepo,FormRepository $formRepo,SocialProvider $socialProvider)
    {
        $this->user = $user;
    ...
    public function changePassword(Request $request){
        ...
        $response = [
            'error'=>'Your password is not changed',
        ];
        return response()->json($response,404);   
    }

    public function updateName($id,Request $request){
       if($this->userRepo->update($id,$request->all())){
            $response = [
    ...
    public function myEventList($user_id){
        //return $this->formRepo->getFormByUserId($user_id);
        if($this->formRepo->getFormByUserId($user_id)){
            $response = [
                'events'=>$this->formRepo->getFormByUserId($user_id),
            ];
            return response()->json($response,200);
       }
    }

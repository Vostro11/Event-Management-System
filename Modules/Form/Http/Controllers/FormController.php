...
class FormController extends Controller
{
    ...
    public function uploadFromFile($form_id){
        //return 'upload from file';
        $form= $this->formRepo->getById($form_id);
        return view('form::upload_from_file',compact('form'));
    }

    public function storeFromFile($form_id,Request $request){
        // Import a user provided file
        $request=$request->all();
        //return $request;
        $data = Excel::load(Input::file('myfile'))->get()->toArray();       
            foreach ($data as $key => $mydata) {
                ...
                $mydata['form_id']= $request['form_id'];
                $mydata['version']= $request['version'];
                }
                $this->formRepo->formSubmit($mydata);
                /*echo "<pre>";
                print_r($mydata);
                echo "</pre>";*/
            }
        Session::flash('success','Value is submited');
        return back();
    }   
